...
.text:6B2D7982
.text:6B2D7982                               loc_6B2D7982:                           ; CODE XREF: LdrpProcessMappedModule(x,x)+160��j
.text:6B2D7982                                                                       ; LdrpProcessMappedModule(x,x)+169��j ...
...
PerformBaseRelocation(PMEMORYMODULE module, ptrdiff_t delta)
{
	...
	for (; relocation->VirtualAddress > 0;)
	{
		...
		for (i = 0; i < ((relocation->SizeOfBlock - IMAGE_SIZEOF_BASE_RELOCATION) / 2); i++, relInfo++)
		{
			...
			{
				*(PULONG_PTR)((PBYTE)dest + offset) += HIWORD(delta);
			}
			break;

			case IMAGE_REL_BASED_LOW:
			{
				*(PULONG_PTR)((PBYTE)dest + offset) += LOWORD(delta);
			}
			break;
...
static BOOL BuildDelayedImportTable(PMEMORYMODULE module)
{
	unsigned char *codeBase = module->codeBase;
	PIMAGE_DELAYLOAD_DESCRIPTOR importDesc;
	BOOL result = TRUE;

	PIMAGE_DATA_DIRECTORY directory = GET_HEADER_DICTIONARY(module, IMAGE_DIRECTORY_ENTRY_DELAY_IMPORT);
	if (directory->Size == 0)
	{
		return TRUE;
	}

	importDesc = (PIMAGE_DELAYLOAD_DESCRIPTOR)(codeBase + directory->VirtualAddress);
	for (; !IsBadReadPtr(importDesc, sizeof(PIMAGE_DELAYLOAD_DESCRIPTOR)) && importDesc->DllNameRVA; importDesc++)
	{
		uintptr_t *thunkRef;
		FARPROC *funcRef;
		HCUSTOMMODULE *tmp;
		HCUSTOMMODULE handle = module->loadLibrary((LPCSTR)(codeBase + importDesc->DllNameRVA), module->userdata);
		if (handle == NULL)
		{
		...
		if (importDesc->ImportNameTableRVA)
		{
			thunkRef = (uintptr_t *)(codeBase + importDesc->ImportNameTableRVA);
			funcRef = (FARPROC *)(codeBase + importDesc->ImportAddressTableRVA);
		}
		else
		{
			// no hint table
			thunkRef = (uintptr_t *)(codeBase + importDesc->ImportAddressTableRVA);
			funcRef = (FARPROC *)(codeBase + importDesc->ImportAddressTableRVA);
		}
